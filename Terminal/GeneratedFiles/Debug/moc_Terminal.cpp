/****************************************************************************
** Meta object code from reading C++ file 'Terminal.h'
**
** Created by: The Qt Meta Object Compiler version 67 (Qt 5.1.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "../../Source/Terminal.h"
#include <QtCore/qbytearray.h>
#include <QtCore/qmetatype.h>
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'Terminal.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 67
#error "This file was generated using the moc from 5.1.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
struct qt_meta_stringdata_Terminal_t {
    QByteArrayData data[15];
    char stringdata[158];
};
#define QT_MOC_LITERAL(idx, ofs, len) \
    Q_STATIC_BYTE_ARRAY_DATA_HEADER_INITIALIZER_WITH_OFFSET(len, \
    offsetof(qt_meta_stringdata_Terminal_t, stringdata) + ofs \
        - idx * sizeof(QByteArrayData) \
    )
static const qt_meta_stringdata_Terminal_t qt_meta_stringdata_Terminal = {
    {
QT_MOC_LITERAL(0, 0, 8),
QT_MOC_LITERAL(1, 9, 17),
QT_MOC_LITERAL(2, 27, 0),
QT_MOC_LITERAL(3, 28, 16),
QT_MOC_LITERAL(4, 45, 19),
QT_MOC_LITERAL(5, 65, 3),
QT_MOC_LITERAL(6, 69, 7),
QT_MOC_LITERAL(7, 77, 4),
QT_MOC_LITERAL(8, 82, 10),
QT_MOC_LITERAL(9, 93, 13),
QT_MOC_LITERAL(10, 107, 3),
QT_MOC_LITERAL(11, 111, 16),
QT_MOC_LITERAL(12, 128, 14),
QT_MOC_LITERAL(13, 143, 5),
QT_MOC_LITERAL(14, 149, 7)
    },
    "Terminal\0updateWidgetsInfo\0\0"
    "changeConnection\0const std::wstring*\0"
    "phy\0connect\0warn\0onInterval\0transmitDebug\0"
    "str\0showWarningConst\0const wchar_t*\0"
    "title\0content\0"
};
#undef QT_MOC_LITERAL

static const uint qt_meta_data_Terminal[] = {

 // content:
       7,       // revision
       0,       // classname
       0,    0, // classinfo
       5,   14, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors
       0,       // flags
       0,       // signalCount

 // slots: name, argc, parameters, tag, flags
       1,    0,   39,    2, 0x08,
       3,    3,   40,    2, 0x08,
       8,    0,   47,    2, 0x08,
       9,    1,   48,    2, 0x08,
      11,    2,   51,    2, 0x08,

 // slots: parameters
    QMetaType::Void,
    QMetaType::Void, 0x80000000 | 4, QMetaType::Bool, QMetaType::Bool,    5,    6,    7,
    QMetaType::Void,
    QMetaType::Void, QMetaType::QString,   10,
    QMetaType::Void, 0x80000000 | 12, 0x80000000 | 12,   13,   14,

       0        // eod
};

void Terminal::qt_static_metacall(QObject *_o, QMetaObject::Call _c, int _id, void **_a)
{
    if (_c == QMetaObject::InvokeMetaMethod) {
        Terminal *_t = static_cast<Terminal *>(_o);
        switch (_id) {
        case 0: _t->updateWidgetsInfo(); break;
        case 1: _t->changeConnection((*reinterpret_cast< const std::wstring*(*)>(_a[1])),(*reinterpret_cast< bool(*)>(_a[2])),(*reinterpret_cast< bool(*)>(_a[3]))); break;
        case 2: _t->onInterval(); break;
        case 3: _t->transmitDebug((*reinterpret_cast< const QString(*)>(_a[1]))); break;
        case 4: _t->showWarningConst((*reinterpret_cast< const wchar_t*(*)>(_a[1])),(*reinterpret_cast< const wchar_t*(*)>(_a[2]))); break;
        default: ;
        }
    }
}

const QMetaObject Terminal::staticMetaObject = {
    { &QMainWindow::staticMetaObject, qt_meta_stringdata_Terminal.data,
      qt_meta_data_Terminal,  qt_static_metacall, 0, 0}
};


const QMetaObject *Terminal::metaObject() const
{
    return QObject::d_ptr->metaObject ? QObject::d_ptr->dynamicMetaObject() : &staticMetaObject;
}

void *Terminal::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Terminal.stringdata))
        return static_cast<void*>(const_cast< Terminal*>(this));
    return QMainWindow::qt_metacast(_clname);
}

int Terminal::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QMainWindow::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        if (_id < 5)
            qt_static_metacall(this, _c, _id, _a);
        _id -= 5;
    } else if (_c == QMetaObject::RegisterMethodArgumentMetaType) {
        if (_id < 5)
            *reinterpret_cast<int*>(_a[0]) = -1;
        _id -= 5;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
