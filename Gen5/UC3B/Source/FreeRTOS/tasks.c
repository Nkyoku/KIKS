...
PRIVILEGED_DATA static volatile portTickType xNextTaskUnblockTime				= ( portTickType ) portMAX_DELAY;
	...
	static void prvGenerateRunTimeStatsForTasksInList( const char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime ) PRIVILEGED_FUNCTION;
 ...
 */
#define tskBLOCKED_CHAR		( ( char ) 'B' )
#define tskREADY_CHAR		( ( char ) 'R' )
#define tskDELETED_CHAR		( ( char ) 'D' )
#define tskSUSPENDED_CHAR	( ( char ) 'S' )
 ...
 */
	...
	static void prvListTaskWithinSingleList( const char *pcWriteBuffer, xList *pxList, char cStatus ) PRIVILEGED_FUNCTION;
...
portBASE_TYPE xReturn;
	...
	#if ( INCLUDE_xTaskGetIdleTaskHandle == 1 )
	{
		...
		xReturn = xTaskCreate( prvIdleTask, ( char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), &xIdleTaskHandle );
	}
	#else
	{
		/* Create the idle task without storing its handle. */
		xReturn = xTaskCreate( prvIdleTask, ( char * ) "IDLE", tskIDLE_STACK_SIZE, ( void * ) NULL, ( tskIDLE_PRIORITY | portPRIVILEGE_BIT ), NULL );
	}
	#endif
...
static void prvInitialiseTCBVariables( tskTCB *pxTCB, const char * const pcName, unsigned portBASE_TYPE uxPriority, const xMemoryRegion * const xRegions, unsigned short usStackDepth )
{
	...
	#endif
	pxTCB->pcTaskName[ ( unsigned short ) configMAX_TASK_NAME_LEN - ( unsigned short ) 1 ] = ( char ) '\0';
...
/*-----------------------------------------------------------*/
	...
	static void prvListTaskWithinSingleList( const char *pcWriteBuffer, xList *pxList, char cStatus )
	{
...
/*-----------------------------------------------------------*/
	...
	static void prvGenerateRunTimeStatsForTasksInList( const char *pcWriteBuffer, xList *pxList, unsigned long ulTotalRunTime )
	{
	volatile tskTCB *pxNextTCB, *pxFirstTCB;
