...
{
	char *pcHead;				/*< Points to the beginning of the queue storage area. */
	char *pcTail;				/*< Points to the byte at the end of the queue storage area.  Once more byte is allocated than necessary to store the queue items, this is used as a marker. */

	char *pcWriteTo;				/*< Points to the free next place in the storage area. */
	char *pcReadFrom;			/*< Points to the last place that a queued item was read from. */
 ...
 */
	...
	void vQueueAddToRegistry( xQueueHandle xQueue, char *pcQueueName ) PRIVILEGED_FUNCTION;
#endif
...
xQueueHandle xReturn = NULL;
	...
	if( uxQueueLength > ( unsigned portBASE_TYPE ) 0 )
	{
		...
		if( pxNewQueue != NULL )
		{
			...
			pxNewQueue->pcHead = ( char * ) pvPortMalloc( xQueueSizeInBytes );
...
/*-----------------------------------------------------------*/
	...
	void vQueueAddToRegistry( xQueueHandle xQueue, char *pcQueueName )
	{
