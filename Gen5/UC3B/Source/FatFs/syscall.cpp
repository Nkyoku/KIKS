...
extern "C" int ff_cre_syncobj (	/* 1:Function succeeded, 0:Could not create due to any error */
...
)
{
	...
	if (!sem[vol]){					/* FreeRTOS */
		...
		vSemaphoreCreateBinary(sem[vol]);
	}
	*sobj = sem[vol];
	ret = (*sobj != NULL);
...
extern "C" int ff_req_grant (	/* TRUE:Got a grant to access the volume, FALSE:Could not get a grant */
...
extern "C" void* ff_memalloc (	/* Returns pointer to the allocated memory block */
